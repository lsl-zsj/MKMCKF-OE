...
load('sensorlog_20200312_174910.mat');
...
gyro =[AngularVelocity.X,AngularVelocity.Y,AngularVelocity.Z];
elu_phone=[Orientation.X,Orientation.Y,Orientation.Z];
acc(1,:)=[];
fs=100;
data=orientation_estimation_fun(acc,gyro,fs);
elu=data.elu;
...
%% calculate using ahrs filter
acc = [Acceleration.X,Acceleration.Y,Acceleration.Z];
gyro =[AngularVelocity.X,AngularVelocity.Y,AngularVelocity.Z];
mag=[MagneticField.X,MagneticField.Y,MagneticField.Z];
acc(1,:)=[];
len=length(acc);
fs=100;
elu_ahrs=orientation_estimation_ahrs_fun(acc,gyro,mag,len,fs);


figure('Name', 'Euler Angles');
hold on;
x1=subplot(3,1,1);
plot(time, euler(:,1),time,elu_ahrs(:,3) ,time, -elu_phone(:,2), 'r');
legend('gradient','ahrs','phone')
x2=subplot(3,1,2);
plot(time, euler(:,2),time,elu_ahrs(:,2), time, elu_phone(:,3),'g');
legend('gradient','ahrs','phone')
x3=subplot(3,1,3);
plot(time, euler(:,3),time,elu_ahrs(:,1),time, -elu_phone(:,1)-90,'b');
legend('gradient','ahrs','phone')
xlabel('Time (s)');
ylabel('Angle (deg)');
linkaxes([x1,x2,x3],'x');
